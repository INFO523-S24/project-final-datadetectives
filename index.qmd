...
def update_plot(year):
    ...
    sns.kdeplot(data = data_year, x = 'renewables_consumption', hue = 'continents', multiple = 'stack', alpha = 0.3, linewidth = 0.2, ax = ax)
    
    
    # Set title
    ax.set_title(f'Density Plot of Renewables Consumption by Continent for Year {year}')
    #### Set the x label
    ax.set_xlabel('Renewable Energy Consumption')

# Create a slider for the years
slider_ax = plt.axes([0.1, 0.0, 0.65, 0.03], facecolor='lightgoldenrodyellow')
slider = Slider(slider_ax, 'Year', years[0], years[-1], valinit=current_year, valstep=1)

# Create a button for playing the animation
button_ax = plt.axes([0.8, 0.02, 0.1, 0.03])
button = Button(button_ax, 'Play', color='lightblue', hovercolor='skyblue')
...
def on_button_click(event):
    global current_year
    current_year = slider.val + 1 if slider.val < years[-1] else years[0]
    slider.set_val(current_year)

# Link the slider and button to their respective update functions
...
def update_plot(year):
    ...
    # Plot the KDE plot for the selected year
    sns.kdeplot(data = data_year, x = 'non_renewables_consumption', hue = 'continents', multiple = 'stack', alpha = 0.3, linewidth = 0.2, ax = ax)
    
    
    # Set title
    ax.set_title(f'Density Plot of Non-Renewables Consumption by Continent for Year {year}')
    #### Set the x label
    ax.set_xlabel('Non-Renewable Energy Consumption')
